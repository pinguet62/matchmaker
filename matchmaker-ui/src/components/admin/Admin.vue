  ...
  export default {
    ...
    methods: {
      async tinderLogin () {
        ...
        let userId = (await axios.post('http://localhost:8081/login', {token: this.tinderToken})).data
        this.userId = userId
      ...
      async loadMatches () {
        this.matches = (await axios.get('http://localhost:8081/matches', {headers: {userId: this.userId}})).data
      },
      async loadSharedLinks () {
        this.sharedLinks = (await axios.get('http://localhost:8081/sharedLinks', {headers: {userId: this.userId}})).data
      },
      async onCreateSharedLink () {
        let createdSharedLink = (await axios.post('http://localhost:8081/sharedLinks', null, {headers: {userId: this.userId}})).data
      ...
      async onDeleteSharedLink () {
        ...
        await axios.delete(`http://localhost:8081/sharedLinks/${sharedLinkLink}`, {headers: {userId: this.userId}})
      ...
      async onSaveSharedLinkMatches () {
        await axios.put(`http://localhost:8081/sharedLinks/${this.selectedSharedLink.link}`, this.selectedSharedLink.matchIds, {headers: {userId: this.userId}})
      }
    }
  }
</script>
